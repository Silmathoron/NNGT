...
* Calling ``nngt.geography`` or ``nngt.simulation`` directly in python causes
  a ``ValueError: module object substituted in sys.modules during a lazy load``
  which I don't know how to avoided... use
  ``from nngt.geography/simulation import whatever_you_want`` or
  ``import nngt.geography/simulation as ng/ns`` instead.
